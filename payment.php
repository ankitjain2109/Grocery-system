	...
	<h1>EMAIL ID :</h1>&nbsp;&nbsp;&nbsp;
		...
		<tr>
			...
			$res=mysql_fetch_array($sql);
					echo "<h3>";
					echo "<p title='Your Address Is From Your Database'>";
					echo $res['first_name']; echo "&nbsp;&nbsp;"; echo $res['last_name'];
					echo "<br/>";
					echo $res['house_no']; echo "&nbsp;&nbsp;"; echo $res['address']; echo "&nbsp;&nbsp;"; 
					echo $res['place']; echo "&nbsp;&nbsp;"; echo $res['state']; echo "&nbsp;&nbsp;"; 
					echo $res['pincode'];
		...
		<tr>
		<td>
	<?php /*?>	<h3>SEND YOUR ITEMS AS A GIFT . PACKING CHARGES EXTRA .</h3></td>
		<td>
		
		<input type="submit" class="effectscale" name="send_gift" value="gift" title="Send Your Friend A Gift Using Daily Needs." style="height:48px; width:162px">
		
		<?php
		
		if($_REQUEST['send_gift'])
		{
				echo "<script>window.location.assign('gift.php')</script>";
		}
		...
		<tr>
			...
			<h2 align="center">
			&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="submit" class="pulse-shrink shadow-radial" id="box" name="checkout_cod" value="COD" title="use your payment mode as cash on delivery" style="height:48px; width:120px"/>
&nbsp;&nbsp;
			<input type="submit" class="pulse-shrink shadow-radial" id="box" name="checkout_debit" value="Debit Card" title="use your payment mode as debit card" style="height:48px; width:120px"/>	
&nbsp;&nbsp;
			<input type="submit" class="pulse-shrink shadow-radial" id="box" name="net_banking" value="Net Banking" title="use your payment mode as net banking" style="height:48px; width:162px">
			</h2>
	</p>
			<br>
			<br>
			...
			if($_REQUEST['checkout_cod'])
			{
				include ("random_function.php");
			}
			else if($_REQUEST['checkout_debit'])
			{
				include ("debit_card.php");
				
			}
			else if($_REQUEST['net_banking'])
			{
				include "net_banking.php";
			}
...
$n1=$_REQUEST['n1'];
$slot=$_REQUEST['slot'];
if($_REQUEST['finalize1'])
{
	if($n=="")
	{
	echo "<script>alert('Enetr The Value')</script>";
	}
	else if($n!=$n1)
	{	
		echo "<script>alert('Captcha Didn't Match')</script>";
	}
	
	else
	{	
		
		mysql_query("update cart_add set timing='$slot',email='$uid' where email='' ");
